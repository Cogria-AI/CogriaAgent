"""DEV ONLY — stand in for a business backend's /agent-auth/exchange.

A real deployment mints the JWT from the user's real session. For the local demo
we mint one unconditionally so the BFF has an exchange endpoint to call. NEVER
